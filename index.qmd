...
Pixi does **not** virtualise the "hardware" and, because of it, you can**not**

- use a different version of C standard library
- duplicate ports

If you are working in a project that requires the above, you should use containers (for example, [Podman](https://podman.io/), [Docker](https://www.docker.com/), [Apptainer](https://apptainer.org/), ...) instead of Pixi.
...
1.  Copy course material:
    ...
    ```bash
    git clone git@github.com:rgaiacs/dra-ttt-course-pixi-mwe.git
    ```

2.  Check tasks created by instructor:
...
4.  Convert Notebook to R script
...
3.  Create alias for tasks:
    ...
    ```bash
    pixi task add jupyter "jupyter lab --no-browser"
    ```
